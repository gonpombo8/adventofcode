...
const challengeA = (sum: number): number[] | undefined => {
  const numberA = numbers.find((number) => {
    const numKey = sum - number;
    const num = hashNumbers[numKey];

    if (numKey === number && num === 1) {
      return false;
    }
  ...
  if (!numberA) {
    console.log('2-Tuple not found');
    return undefined;
  }

  const numberB = sum - numberA;
  console.log(
    `2-Tuple found: ${numberA} - ${numberB}`,
    `Multiply: ${numberA * numberB}`,
  );
  return [numberA, numberB];
};

const challengeB = (sum: number) => {
  const numbersResult = numbers.reduce(
    (acc, numberA) => {
      const numberB = numbers.find((n) => {
        const numKey = sum - numberA - n;
        const num = hashNumbers[numKey];
        return !!num;
      });

      if (numberB) {
        return acc.concat([[numberA, numberB, sum - numberA - numberB]]);
      }
  ...
  if (!numbersResult.length) {
    console.log('3-Tuple not found');
    return undefined;
  }

  numbersResult.forEach((numbers) => {
    if (numbers.length) {
      console.log(
        `3-Tuple found: ${numbers}`,
        `Multiply: ${numbers.reduce((acc, n) => n * acc, 1)}`,
      );
    }
  })
};
